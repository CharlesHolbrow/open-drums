...
const pluginTEqualiser = {
    ...
    setBand1(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand1FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand1GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand1Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand1Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand2(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand2FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand2GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand2Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand2Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand3(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand3FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand3GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand3Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand3Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand4(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand4FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand4GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand4Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand4Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand5(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand5FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand5GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand5Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand5Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand6(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand6FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand6GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand6Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand6Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand7(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand7FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand7GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand7Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand7Active(active));
        return msg;
    },
    /**
     * Configure band frequency, gain, and Q
    ...
    setBand8(hz, gain, q, active = true) {
        const msg = [];
        if (typeof hz === 'number')
            msg.push(pluginTEqualiser.setBand8FrequencyHz(hz));
        if (typeof gain === 'number')
            msg.push(pluginTEqualiser.setBand8GainDbfs(gain));
        if (typeof q === 'number')
            msg.push(pluginTEqualiser.setBand8Q(q));
        if (typeof active === 'boolean')
            msg.push(pluginTEqualiser.setBand8Active(active));
        return msg;
    },
    /**
    ...
    setBand1Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 1 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand2Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 2 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand3Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 3 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand4Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 4 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand5Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 5 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand6Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 6 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand7Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 7 State', 0.5, timeInWholeNotes, curve);
    ...
    setBand8Active(value = true, timeInWholeNotes, curve) {
        if (typeof (value) !== "boolean")
            throw new Error("value required to be of type Boolean");
        if (value)
            return fluid.plugin.setExternalParamHelper('Band 8 State', 0.5, timeInWholeNotes, curve);
...
//# sourceMappingURL=plugin-tequaliser.js.map
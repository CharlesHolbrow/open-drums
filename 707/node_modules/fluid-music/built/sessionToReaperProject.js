 ...
 */
function sessionToReaperProject(session, client) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!session.bpm)
            throw new Error('Cannot create reaper project: session BPM must be non-zero');
        ...
        session.forEachTrack((track, i, ancestors) => {
            ...
            const isLastInArray = (item, array) => {
            ...
            for (const [name, automation] of Object.entries(track.automation)) {
                ...
                for (const autoPoint of automation.points) {
                    if (typeof autoPoint.value === 'number') {
                        autoObject.addBezierPoint(autoPoint.startTime * 4 * 60 / session.bpm, normalize(autoPoint.value), autoPoint.curve);
                    }
                }
            ...
            if (fluidTrack.plugins.length > 0) {
                ...
                for (const plugin of fluidTrack.plugins) {
                    ...
                    if (!tracktionSessionActivated) {
                        yield client.send(cybr.global.activate('reaper-helper.tracktionedit', true));
                        tracktionSessionActivated = true;
                    }
                    try {
                        const vst2 = yield vst2ToReaperObject_1.vst2ToReaperObject(client, fluidTrack.name, plugin, nth(plugin), session.bpm);
                        FXChain.add(vst2);
                    }
                    catch (error) {
                        console.warn((chalk.reset `{bold.red WARNING:} fluid-music failed to export the {bold ${plugin.pluginName} (${plugin.pluginType})} plugin.
...
`));
                    ...
                    if (receive instanceof FluidTrack_1.FluidReceive) {
                        if (receive.gainDb !== 0) {
                        ...
                        if (receive.tap !== fluid_interfaces_1.Tap.postFader) {
                        ...
                        if (!found)
                            throw new Error('Failed to find sidechain receive track');
        ...
        if (client && tracktionSessionActivated && closeClientWhenDone)
            client.close();
        return reaperProject;
    });
}
exports.sessionToReaperProject = sessionToReaperProject;
;
...
;
function fileEventsToReaperObjects(fileEvents, session) {
    const bpm = session.bpm;
    if (!bpm)
        throw new Error('fileEventsToReaperObjects could not find a session.bpm parameter');
    return fileEvents.map((audioFile, eventIndex) => {
        ...
        const clipName = `${path_1.basename(audioFile.path)}.${eventIndex}`;
        ...
        const extension = path_1.extname(audioFile.path).toLowerCase();
...
//# sourceMappingURL=sessionToReaperProject.js.map
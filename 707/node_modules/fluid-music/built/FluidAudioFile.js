...
exports.resolveFades = exports.FluidAudioFile = exports.AudioFileMode = void 0;
const converters_1 = require("./converters");
/** AudioFile playback modes */
var AudioFileMode;
(function (AudioFileMode) {
    /**
    ...
    AudioFileMode[AudioFileMode["Event"] = 1] = "Event";
    ...
    AudioFileMode[AudioFileMode["OneShot"] = 2] = "OneShot";
    /**
    ...
    AudioFileMode[AudioFileMode["OneVoice"] = 3] = "OneVoice";
})(AudioFileMode = exports.AudioFileMode || (exports.AudioFileMode = {}));
/**
...
class FluidAudioFile {
    ...
    setMarkers(markers) {
        if (markers instanceof Map) {
            for (const [key, value] of markers.entries())
    ...
    getMaxDurationSeconds() {
        if (this.isReversed())
            return this.startInSourceSeconds / this.playbackRate * -1;
    ...
    getTailRightSeconds() {
        if (this.isReversed())
            return this.getEndInSourceSeconds() / this.playbackRate * -1;
    ...
    getTailLeftSeconds() {
        if (this.isReversed())
            return (this.startInSourceSeconds - this.getSourceDurationSeconds()) / this.playbackRate;
     ...
     * `this.playbackRate != 1`).
     */
    growRightEdgeBySeconds(seconds) {
        this.durationSeconds += seconds;
    }
    growRightEdgeBySecondsSafe(seconds) {
    ...
    playToEnd() {
        ...
        if (warning)
            console.warn('Warning:', warning, this);
    }
}
exports.FluidAudioFile = FluidAudioFile;
FluidAudioFile.Modes = AudioFileMode;
/**
 ...
 * `.durationSeconds`.
 */
function resolveFades(audioFile) {
    const fractionOfFadeInTime = converters_1.clamp(0, 1, audioFile.durationSeconds / audioFile.fadeInSeconds);
    const fadeInGain = Math.sqrt(fractionOfFadeInTime); // Assume constant power fade
    const fadeInDb = converters_1.gainToDb(fadeInGain);
    const fractionOfFadeOutTime = converters_1.clamp(0, 1, audioFile.durationSeconds / audioFile.fadeOutSeconds);
    const fadeOutGain = Math.sqrt(fractionOfFadeOutTime); // Assume constant power fade
    const fadeOutDb = converters_1.gainToDb(fadeOutGain);
...
exports.resolveFades = resolveFades;
//# sourceMappingURL=FluidAudioFile.js.map
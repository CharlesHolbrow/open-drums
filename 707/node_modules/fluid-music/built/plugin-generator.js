...
exports.generatePluginModule = void 0;
const plugin_report_utils_1 = require("./cybr/plugin-report-utils");
const plugin_generator_name_helpers_1 = require("./plugin-generator-name-helpers");
/**
 ...
 */
function generatePluginModule(report, config = {}) {
    ...
    const pluginTypeExt = plugin_generator_name_helpers_1.upperFirstLetter(pluginType.split('.')[1].toLowerCase()); // Vst2
    const className = typeof config.className === 'string' ? config.className : plugin_generator_name_helpers_1.makeVarName(pluginName) + pluginTypeExt; // DragonflyRoomVst2
    ...
    params.forEach(paramInfo => {
        if (!paramInfo.hasOwnProperty('guess'))
            paramInfo.guess = plugin_report_utils_1.guess(paramInfo);
        if (!paramInfo.hasOwnProperty('index'))
            paramInfo.index = paramInfo.tracktionIndex - 2;
        // Assume guesses are correct iff the underlying object does not specify
        for (const k of Object.keys(paramInfo.guess)) {
            if (!paramInfo.hasOwnProperty(k))
                paramInfo[k] = paramInfo.guess[k];
        }
        // If we have a normalizer we can make a better key by adding units
        if (!paramInfo.hasOwnProperty('key')) {
            paramInfo.key = plugin_generator_name_helpers_1.camelCaseFromParamName(paramInfo.name);
            if (paramInfo.units && (paramInfo.isLinear || typeof paramInfo.powerFuncB === 'number')) {
                paramInfo.key += plugin_generator_name_helpers_1.upperFirstLetter(paramInfo.units);
            }
        }
    });
...
`;
    ...
    output += params.map(param => {
        let result = '';
        if (param.range)
            result += `  /** ${param.units ? param.units + ' ' : ''}value from ${param.range[0]} to ${param.range[1]} */\n`;
    ...
    output += params.map(param => {
        ...
        if (param.range)
            result += `, range: [${param.range[0]}, ${param.range[1]}] as [number, number]`;
        if (param.units)
            result += `, units: '${param.units}'`;
        if (typeof param.powerFuncB === 'number')
            result += `, powerFuncB: ${param.powerFuncB}`;
        if (param.choices)
            result += `, choices: ${JSON.stringify(param.choices)}`;
        if (param.normalizeFunctionAsString)
            result += `,
...
exports.generatePluginModule = generatePluginModule;

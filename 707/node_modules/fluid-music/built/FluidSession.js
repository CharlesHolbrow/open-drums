...
exports.parse = exports.FluidSession = void 0;
const path = require("path");
const chalk = require("chalk");
const tab = require("./tab");
const cybr = require("./cybr");
const FluidTrack_1 = require("./FluidTrack");
const sessionToFluidMessage_1 = require("./sessionToFluidMessage");
const sessionToReaperProject_1 = require("./sessionToReaperProject");
...
class FluidSession {
    constructor(session, tracks = []) {
        ...
        if (typeof session.bpm === 'number')
            this.bpm = session.bpm;
        if (typeof session.loopDuration === 'number')
            this.loopRegion.duration = session.loopDuration;
        if (typeof session.loopStartTime === 'number')
            this.loopRegion.startTime = session.loopStartTime;
        if (typeof session.loopEnabled === 'boolean')
            this.loopEnabled = session.loopEnabled;
        else if (typeof session.loopDuration === 'number')
            this.loopEnabled = true;
        if (session.d)
            this.scoreConfig.d = session.d;
        if (session.dLibrary)
            this.scoreConfig.dLibrary = session.dLibrary;
        if (session.r)
            this.scoreConfig.r = session.r;
        if (session.tLibrary)
            this.scoreConfig.tLibrary = session.tLibrary;
        if (session.trackKey)
            this.scoreConfig.trackKey = session.trackKey;
        if (session.startTime)
            this.scoreConfig.startTime = session.startTime;
        for (const [name, track] of Object.entries(tracks)) {
            if (!track.name)
                track.name = name;
            const newTrack = (track instanceof FluidTrack_1.FluidTrack)
                ? track
                : new FluidTrack_1.FluidTrack(track);
    ...
    forEachTrack(func) {
        ...
        const iterateOver = (tracks, ancestors) => {
            let i = 0; // parentIndex
    ...
    getOrCreateTrackByName(name) {
        const track = this.getTrackByName(name);
        if (track)
            return track;
        const newTrack = new FluidTrack_1.FluidTrack({ name });
        this.tracks.push(newTrack);
        return newTrack;
    }
    /**
     ...
     * @param name
     */
    getOrCreateTrackAncestorsByName(name) {
     ...
     * @chainable
     */
    insertScore(score, config = {}) {
        config = Object.assign({ startTime: this.editCursorTime }, config);
    ...
    resolveSends() {
        this.forEachTrack((sendTrack, i) => {
            if (!sendTrack.unresolvedSends.length)
                return;
            sendTrack.unresolvedSends = sendTrack.unresolvedSends.filter(send => {
                ...
                if (receiveTrack) {
                    ...
                    const receive = new FluidTrack_1.FluidReceive(receiveOptions);
        ...
        this.forEachTrack(track => {
            for (const plugin of track.plugins) {
                if (plugin.unresolvedSidechainReceive) {
                    ...
                    if (sendTrack) {
                        plugin.sidechainReceive = new FluidTrack_1.FluidReceive({
                            from: sendTrack,
    ...
    timeWholeNotesToSeconds(timeInWholeNotes) {
        return timeInWholeNotes * 4 * 60 / this.bpm;
    }
    timeSecondsToWholeNotes(timeInSeconds) {
        ...
        this.forEachTrack(track => {
            track.clips.forEach((clip, clipIndex) => {
                ...
                for (const event of clip.events) {
                    if (typeof ((_a = event.technique) === null || _a === void 0 ? void 0 : _a.use) === 'function') {
                        let useContext = {
                            d: event.d ? event.d : {},
                            track,
                            clip,
    ...
    finalize() {
        ...
        this.forEachTrack(track => {
            ...
            for (let i = 1; i < track.audioFiles.length; i++) {
                ...
                if (fEvent1.mode === FluidAudioFile_1.AudioFileMode.OneVoice && typeof fEvent1.info.duration === 'number') {
     ...
     */
    saveAsTracktionFile(filename = 'session', client) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!client) {
                ...
                yield client.connect(true);
            }
            yield this.sendToServer(filename, client);
            yield new Promise(resolve => setTimeout(resolve, 100));
            yield client.send(cybr.global.save());
     ...
     *    configured with `keepOpen=true`.
     */
    sendToServer(filename = 'session', client) {
        return __awaiter(this, void 0, void 0, function* () {
            let closeWhenFinished = false;
            // create a client if caller did not provide one
            if (!client) {
            ...
            yield client.send([
                ...
                sessionToFluidMessage_1.sessionToTemplateFluidMessage(this),
                sessionToFluidMessage_1.sessionToContentFluidMessage(this),
            ]);
            if (closeWhenFinished)
                client.close();
        });
     ...
     *    when not provided
     */
    saveAsReaperFile(filename = 'fluid', client) {
        return __awaiter(this, void 0, void 0, function* () {
            // create a filename if the caller did not provide one
            if (!filename)
                filename = 'fluid';
            ...
            if (!filename.toLowerCase().endsWith('.rpp'))
                filename += '.RPP';
            const rpp = yield sessionToReaperProject_1.sessionToReaperProject(this, client);
            const stream = fs_1.createWriteStream(filename, 'utf8');
            stream.write(rpp.dump());
        });
    }
}
exports.FluidSession = FluidSession;
/**
 ...
 * @internal
 */
function parse(scoreObject, session = new FluidSession({}), config = {}) {
    ...
    if (scoreObject.hasOwnProperty('tLibrary'))
        config.tLibrary = Object.assign((config.tLibrary && Object.assign({}, config.tLibrary)) || {}, scoreObject.tLibrary);
    if (scoreObject.hasOwnProperty('dLibrary'))
        config.dLibrary = Object.assign((config.dLibrary && Object.assign({}, config.dLibrary)) || {}, scoreObject.dLibrary);
    if (scoreObject.hasOwnProperty('r'))
        config.r = scoreObject.r;
    if (scoreObject.hasOwnProperty('d'))
        config.d = scoreObject.d;
    ...
    if (Array.isArray(scoreObject)) {
        ...
        const [trackTLibrary, trackDLibrary] = trackAncestry.reduce(([prevTLibrary, prevDLibrary], track) => {
        ...
        if (r === undefined) {
            const msg = `Encountered a pattern ("${scoreObject}") on a track (${track === null || track === void 0 ? void 0 : track.name}), but could not find a .r rhythm`;
            throw new Error(msg);
        }
        // create the clip
        let rhythmObject;
...
{green track:}   "${track === null || track === void 0 ? void 0 : track.name}"
{green rhythm:}  "${r}"
...
`));
        ...
        if (d) {
            ...
            for (const event of resultClip.events)
                event.d = getDynamic(event.startTime);
        }
        ...
        for (let [key, val] of Object.entries(scoreObject)) {
            if (tab.reservedKeys.hasOwnProperty(key) && key !== 'clips')
                continue;
            if (key !== 'clips')
                config.trackKey = key; // if key='clips' use parent key
            let r = parse(val, session, config);
            if (r.duration > result.duration)
                result.duration = r.duration;
        }
    }
    return result;
...
exports.parse = parse;
;
//# sourceMappingURL=FluidSession.js.map
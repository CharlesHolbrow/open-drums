...
exports.sessionToContentFluidMessage = exports.sessionToTemplateFluidMessage = void 0;
...
const isSubmixTrack = (track) => {
    return !!track.children.length;
};
const createSelectMessage = (track, parentName) => {
 ...
 * @param session
 */
function sessionToTemplateFluidMessage(session) {
    ...
    session.forEachTrack((track, i, ancestors) => {
        ...
        const trackMessages = [
            ...
            cybr.audiotrack.gain(track.gainDb),
    ...
    session.forEachTrack((track, i, ancestors) => {
        const parentName = ancestors.length ? ancestors[ancestors.length - 1].name : undefined;
        const trackMessages = [createSelectMessage(track, parentName)];
        sessionMessages.push(trackMessages);
        ...
        const allPluginMessages = [];
        trackMessages.push(allPluginMessages);
        for (const plugin of track.plugins) {
            const pluginMessages = [];
            allPluginMessages.push(pluginMessages);
            const cybrType = plugin.pluginType === FluidPlugin_1.PluginType.unknown ? null : plugin.pluginType;
            ...
            if (plugin.sidechainReceive) {
                ...
                if (plugin.sidechainReceive.tap !== fluid_interfaces_1.Tap.postFader) {
    ...
    session.forEachTrack(track => {
        if (!track.receives.length)
            return;
        const sendReceiveMessage = [];
        ...
        for (const receive of track.receives) {
            sendReceiveMessage.push(cybr.audiotrack.select(receive.from.name), cybr.audiotrack.send(track.name, receive.gainDb));
        }
    });
    return sessionMessages;
}
exports.sessionToTemplateFluidMessage = sessionToTemplateFluidMessage;
/**
 * Create a `FluidMessage` from a `FluidSession`
 *
 * @param session object generated by score.parse
 */
function sessionToContentFluidMessage(session) {
    const sessionMessages = [];
    // Configure loop region
    if (session.loopEnabled) {
        sessionMessages.push(cybr.transport.loop(session.loopRegion.startTime, session.loopRegion.startTime + session.loopRegion.duration));
    }
    else {
    ...
    session.forEachTrack((track, i, ancestors) => {
            ...
            for (const autoPoint of automation.points) {
                if (typeof autoPoint.value === 'number') {
                    ...
                    if (name === 'gain' || name === 'gainDb')
                        msg = cybr.audiotrack.gain(autoPoint.value, autoPoint.startTime, autoPoint.curve);
                    else if (name === 'pan')
                        msg = cybr.audiotrack.pan(autoPoint.value, autoPoint.startTime, autoPoint.curve);
                    else if (name === 'width')
                        msg = cybr.audiotrack.width(autoPoint.value, autoPoint.startTime, autoPoint.curve);
                    else
        ...
        for (const plugin of track.plugins) {
            const cybrType = (plugin.pluginType === FluidPlugin_1.PluginType.unknown) ? undefined : plugin.pluginType;
            ...
            for (const [paramKey, automation] of Object.entries(plugin.automation)) {
                ...
                for (const autoPoint of automation.points) {
                    if (typeof autoPoint.value === 'number') {
                        ...
                        const normalizedValue = plugin.getNormalizedValue(paramKey, explicitValue);
                        if (typeof normalizedValue === 'number') {
                            trackMessages.push(cybr.plugin.setParamNormalizedAt(paramName, Math.max(Math.min(normalizedValue, 1), 0), autoPoint.startTime, autoPoint.curve));
                        }
                        else {
                            trackMessages.push(cybr.plugin.setParamExplicitAt(paramName, explicitValue, autoPoint.startTime, autoPoint.curve));
                        }
                    }
...
exports.sessionToContentFluidMessage = sessionToContentFluidMessage;
function fileEventToFluidMessage(audioFile, session, summativeIndex) {
    ...
    if (trimFromStart) {
        ...
        if (isReversed)
            sOff = audioFile.getSourceDurationSeconds();
        else
            sOff = 0;
    }
    ...
    const clipName = `${path_1.basename(audioFile.path)}.${summativeIndex}`;
    ...
    const { fadeInSeconds, fadeOutSeconds, gainDb } = FluidAudioFile_1.resolveFades(audioFile);
...
//# sourceMappingURL=sessionToFluidMessage.js.map
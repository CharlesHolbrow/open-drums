...
exports.reservedKeys = exports.createDynamicGetter = exports.patternToSymbolsAndCounts = exports.advanceArrayToSegments = exports.rhythmToAdvanceArray = exports.parseTab = exports.parseRhythm = void 0;
/**
...
exports.parseRhythm = parseRhythm;
;
/**
 ...
 */
function parseTab(rhythm, nPattern, tLibrary) {
 ...
 */
function rhythmToAdvanceArray(rhythm) {
    if (typeof rhythm === 'string')
        rhythm = Array.from(rhythm);
    const result = [];
...
exports.rhythmToAdvanceArray = rhythmToAdvanceArray;
/**
 ...
 * @param advances
 */
function advanceArrayToSegments(advances) {
    const nonZeroIndices = []; // [0, 4]
    advances.forEach((e, i) => { if (e !== 0)
...
exports.advanceArrayToSegments = advanceArrayToSegments;
/**
 ...
 */
function getSegmentStartTotals(advances) {
    const result = [];
    let accumulator = 0;
    advances.forEach(v => {
        if (!v)
...
exports.patternToSymbolsAndCounts = patternToSymbolsAndCounts;
function arrayToSymbolsAndCounts(chars) {
    ...
    if (currentChar === '-')
        throw new Error(`Bad pattern string: "${chars}" (begins on -)`);
    chars.forEach((c) => {
        if (c === '-') {
            if (!onSymbol)
                throw new Error(`Bad pattern string: "${chars}" (- may not follow .)`);
            results[results.length - 1][1]++;
        }
        else if (c === '.') {
            if (onSymbol || !results.length)
                results.push([c, 1]);
            else
                results[results.length - 1][1]++;
            onSymbol = false;
 ...
 * @param dLibrary
 */
function createDynamicGetter(rhythm, dPattern, dLibrary) {
    ...
    if (dPattern.length > rLength)
        dPattern = dPattern.slice(0, rLength);
    ...
    return (time) => {
        ...
        return event === null || event === void 0 ? void 0 : event.technique;
    };
}
exports.createDynamicGetter = createDynamicGetter;
;
/**
 * These keys cannot be used for patterns in tabs and scores.
 */
exports.reservedKeys = {
    r: true,
    d: true,
    v: true,
    noteLibrary: true,
